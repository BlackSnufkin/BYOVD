...
use winapi::um::{
    ...
    securitybaseapi::{CheckTokenMembership, CreateWellKnownSid},
...
use winapi::um::winnt::{GENERIC_EXECUTE, GENERIC_READ, GENERIC_WRITE, SECURITY_MAX_SID_SIZE, WinLocalSystemSid};
...
impl DriverConfig for BdApiUtilDriver {
    const NAME: &'static str = "STProcessMonitor";
    const PATH: &'static str = "\\STProcessMonitor.sys";
    const DEVICE: &'static str = "\\\\.\\STProcessMonitorDriver";
    const IOCTL_CODE: DWORD = 0xB822A00C;
}
...
fn ensure_running_as_local_system() -> Result<()> {
    ...
    if is_member == 0 {
        eprintln!("[X] Please run as LocalSystem (S-1-5-18)");
        return Err("Process is not running as LocalSystem".into());
    }
...
fn main() -> Result<()> {
    let matches = App::new("BYOVD Process Killer")
        .version("1.0")
        .author("BlackSnufkin, wwwab")
        .about("Kills a process by name using STProcessMonitor driver")
        .arg(
            Arg::new("process_name")
                .short("n")
                .long("name")
                .takes_value(true)
                .required(true),
    ...
    if let Err(e) = ensure_running_as_local_system() {
        eprintln!("[X] {}", e);
        process::exit(1);
    }
